...
use nearlend_nft_contract::ContractContract as Contract;
...
pub fn init() -> (UserAccount, ContractAccount<Contract>, UserAccount) {
    ...
    let treasury = root.create_user("treasury".to_string(), to_yocto("100"));
    ...
    root.create_user("test".repeat(16), to_yocto("100"));
...
fn simulate_create_new_series() {
    ...
    println!(
        "[CREATE SERIES] Storage price: {} yoctoNEAR",
        storage_price_for_adding_series
    );
    println!(
        "[CREATE SERIES] Gas burnt price: {} TeraGas",
        outcome.gas_burnt() as f64 / 1e12
    );
}
...
fn simulate_mint() {
    ...
    let storage_price_for_mint: u128 =
        (nft.account().unwrap().storage_usage - initial_storage_usage) as u128 * 10u128.pow(19);
    println!(
        "[MINT 2nd] Storage price: {} yoctoNEAR",
        storage_price_for_mint
    );
    println!(
        "[MINT 2nd] Gas burnt price: {} TeraGas",
        outcome.gas_burnt() as f64 / 1e12
    );
    ...
    let storage_price_for_mint =
        (nft.account().unwrap().storage_usage - initial_storage_usage) as u128 * 10u128.pow(19);
    println!(
        "[MINT 3nd] Storage price: {} yoctoNEAR",
        storage_price_for_mint
    );
    println!(
        "[MINT 3nd] Gas burnt price: {} TeraGas",
        outcome.gas_burnt() as f64 / 1e12
    );
}
...
fn simulate_approve() {
    ...
    let storage_price_for_approve =
        (nft.account().unwrap().storage_usage - initial_storage_usage) as u128 * 10u128.pow(19);
    println!(
        "[APPROVE] Storage price: {} yoctoNEAR",
        storage_price_for_approve
    );
    println!(
        "[APPROVE] Gas burnt price: {} TeraGas",
        outcome.gas_burnt() as f64 / 1e12
    );
}
